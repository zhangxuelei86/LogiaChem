...
public class UIHandler : MonoBehaviour {
    ...
    protected void InitializeFirebaseDB() {
    ...
    protected void StartListener() {
        FirebaseDatabase.DefaultInstance.GetReference("users/" + userData.GetComponent<UserData>().user_id + "/atoms").GetValueAsync().ContinueWith(task => {
            if (task.IsFaulted) {
                // Handle the error...
            }
            else if (task.IsCompleted) {
                easySnap = task.Result;
    ...
    void AuthStateChanged(object sender, System.EventArgs eventArgs) {
        ...
        if (senderAuth == auth && senderAuth.CurrentUser != user) {
            ...
            if (signedIn) {
                ...
                userData.GetComponent<UserData>().user_id = user.UserId;
    ...
    void GUIDisplayControls() {
        ...
        else if (UIEnabled && easyAuth == true) {
            controlsScrollViewVector = GUILayout.BeginScrollView(controlsScrollViewVector);
            GUILayout.BeginVertical();
            GUILayout.Space(20);
            foreach (var item in easySnap.Children) {
                if (GUILayout.Button(item.Child("name").Value.ToString())) {
                    userData.GetComponent<UserData>().chosen_mol = item.Child("molFile").Value.ToString();
